...
> 这张表同样适用于**注入用主相机**的位姿选择：主相机决定 `WorldToScreenPoint` 的坐标映射（序列屏幕坐标的换算依据），选择标准与录像相机一致——覆盖全部关键交互元素、固定不抖动、正交优先。**为当前游戏确定合适的主相机位姿后，把配置（disable_script / position / rotation / FOV / 分辨率）写入序列 header 固化**，回放端逐参数复刻（见 `sequence-record-replay.md`）。

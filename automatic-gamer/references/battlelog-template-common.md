...
| result | 通关/失败 |
| duration_seconds | 用时 |
| progress | **进度梯度指标（必填，见下）** |
| 关键数值指标 | 由类型深度层定义（HP变化、伤害来源分布等精确数值） |

#### progress —— 进度梯度指标（所有游戏类型必填）

> **为什么必须有**：win/lose 是二值结果，对一个"还赢不了"的 bot 几乎没有梯度信息——连输 10 局都是 `lose`，无法判断改动是变好还是变差。调优需要一个**朝胜利方向单调递增的连续/准连续指标**，否则前期就是盲调。

`summary.progress` 至少包含一个**朝着该游戏胜利条件递增的量**，并尽量配套"吞吐/效率"量：

| 字段 | 说明 | 取值来源 |
|------|------|---------|
| primary | 最能代表"离通关多近"的单一数值 | 由胜利条件决定：清房间类=已清房间/波次数；积分类=得分；推进类=到达距离/进度% |
| primary_max | 该指标的目标值（通关所需） | 如总房间数、目标分 |
| throughput | 单位时间的产出效率（可选但强烈建议） | 如击杀数、击杀/分钟、得分/分钟 |
| notes | 进度停滞在哪、卡在第几波/第几关 | 自然语言 |

**进度指标必须真实插桩、可信**（见 SKILL.md 步骤6"先验插桩可信"）。常见反面教材：只记 result + duration，结果"存活时间长"被误当成"接近通关"，而实际 bot 在原地苟活、零推进。

> 纯 log 迭代（无 VLM）时，`progress`/`throughput` 是最重要的优化信号——详见 `references/log-only-diagnosis.md`。

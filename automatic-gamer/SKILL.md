...
description: 基于游戏代码创建Unity游戏自动化助手（Auto Bot）——自动游玩关卡、收集战斗数据、验证关卡可通过性、平衡难度曲线。当用户提到自动化游戏、自动游玩、游戏测试bot、关卡平衡测试、游戏AI助手、auto play、game automation、关卡难度曲线、战斗log分析、run sweep、让游戏自己跑、游戏自动化测试、Unity游戏bot、关卡自动通关、视频分析、键鼠操作序列、序列录制与回放、内挂与序列等价性对比时使用此skill。即使只是想"让游戏自己跑起来看看"，也应触发此skill。
---
...
需要识别：
- 输入系统类型（新 Input System / 旧 Input API），与 3.2 的输入控制实现一致
- 输入注入点（如新输入系统下的 `IInputManager.MovementValue` 只读属性、InputAction 触发方式）
...
{"actions": [
  ...
  {"id": "ui_click", "type": "mouse_click", "target": "任意UI元素（经 EventSystem 射线检测）"},
...
| 输入控制器 | 将决策引擎的输出翻译为步骤1词汇表中的输入原语事件 | 步骤1输入系统分析 |
| **键鼠注入器（InputInjector）** | **唯一注入口**——所有输入事件经它写入游戏输入系统（录制/回放都挂这里，禁止绕过） | 步骤1词汇表 |
...
- **replay 模式**：SequenceReplayer 禁用决策引擎，读取 `sequence.json`，按**帧索引**（`Time.frameCount`，不用墙钟毫秒）重放事件驱动游戏——用于等价性验证：对比"内挂执行"与"序列回放执行"是否一一对应。

序列事件格式（完整 schema 见 `references/sequence-record-replay.md`）：

```json
{"frame": 1204, "t_unscaled": 12.83, "device": "mouse",
 "action": "press", "button": "left", "pos": [256, 480],
 "inject": "queue_state_event"}
```

**录制规则（保证可回放）**：
- 唯一注入口：所有输入事件（移动轴、按键、鼠标）必须走 InputInjector，任何绕过注入口的散装输入调用都会让序列缺失对应项
- 帧索引对齐：记录 `Time.frameCount` 而非墙钟时间，回放时逐帧对齐，不受帧率波动影响
- 记录注入方式：`inject` 字段标注事件走了哪条注入路径（如 `queue_state_event` / `warp_cursor` / `execute_events`），回放必须用同一方式——例如 bot 用 `ExecuteEvents` 直调的 UI 点击，回放改成"坐标+真实鼠标事件"就不严格等价（有 UI 遮挡时行为不同）
...
> **与键鼠序列等价性验证的冲突（必须处理）**：逻辑层直调在键鼠序列里**没有对应物**——回放时无法用键鼠事件复现"调 manager 应用选择 + 关面板"。因此 **`-run-mode replay` 或需要等价性验证时，阻塞 UI 必须改走输入路径**（WarpCursorPosition + 鼠标按下/抬起注入，UI 回调的渲染副作用用 `-batchmode` 不加 `-nographics` 规避），逻辑层直调仅作为非等价模式的优化保留。等价模式下强制关闭看门狗仍然保留。
...
- seed（数字）：固定随机种子（UnityEngine.Random 与 System.Random 同时初始化），写入战斗 log；等价性验证必填
...
4. **结论判定**：
   - 三层全一致 → 内挂与序列一一对应成立
   - 输入流一致但轨迹分歧 → 游戏非确定性（先查 seed/fresh run/speed/timeScale/unscaledTime 依赖，按 `references/sequence-record-replay.md` 的确定性清单逐项排查）
